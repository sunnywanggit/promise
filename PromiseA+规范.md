...
1. 参数要求
    ...
    1.1 onFulfilled 必须是函数类型, 如果不是函数, 应该被忽略(我们不会管传入的是什么值，而是会在 Promise 内部给一个替代的函数,给他一个默认值).
...
6. 返回值
    ...
    6.2 如果 onFulfilled 或者 onRejected 执行时抛出异常e, promise2(也就是返回值)需要被reject
    6.3 如果 onFulfilled 不是一个函数, promise2 以promise1的value 触发fulfilled(如果 onFullied 不是一个函数，我们就要把 promise1 返回的结果原封不动的返回给 promise2)
...
7. resolvePromise
   ...
   ```
    ...
    7.1 如果 promise2 和 x 相等，那么 reject TypeError,不 reject 的话，会造成相互的引用，因为他们两个是同一个东西
