...
class MPromise {
    ...
    resolvePromise(newPromise, x, resolve, reject) {
        ...
        if (x instanceof MPromise) {
            ...
            // promise A+ 规范里面认为下面的写法可能会报错，所以需要使用 try catch 包裹一下
            try {
                // 把 x.then 赋值给 then 
                then = x.then;
            } catch (error) {
            ...
            if (this.isFunction(then)) {
                // onFulfilled 和 onRejected 只能被执行一次，
                // 所以我们这里使用 called 来标识这个函数是否被执行过
                let called = false;
    ...
    // 这里是静态方法(类方法），可以直接通过 MPromise.resolve 直接去调用 
    // 存储对象是这样的 MPromise.a = () => {}
    // 而实例方法只能通过实例进行调用(实例方法),
    // 存储对象是这样的 MPromise.prototype.a = () => {}
